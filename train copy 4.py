...
def extract_plan(best_pos, require_replanning):
    ...
    if best_pos == None:
        ...
        return actions, require_replanning
...
def start_search(env, starting_state, starting_save_num, latest_save_num, starting_repetitions):
    print("start_search")
    start_pos = AStarNode   (  
                                parent=None,
                                dist_x=0,
...
def search(env, pos_pool, best_pos, furthest_pos, current_starting_pos_x, latest_save_num, visited_states, require_replanning):
    ...
    while len(pos_pool) != 0 and (((best_pos.pos_x - current_starting_pos_x) < max_right) or not current_good) and env.episode_length < 600:
...
def optimise(env, best_pos, furthest_pos, pos_pool, current_starting_pos_x, original_state, original_save_num, visited_states, require_replanning, current_action_plan, ticks_before_replanning):
    print("==================================================================================")
    print("==================================================================================")
    print("optimise")
    ...
    print("1 extract_plan(best_pos, require_replanning): " + str(extract_plan(best_pos, require_replanning)))

    state = original_state
    # if ticks_before_replanning <= 0 or len(current_action_plan) == 0 or require_replanning:
    print("ticks_before_replanning: " + str(ticks_before_replanning))
    print("len(current_action_plan): " + str(len(current_action_plan))) 
    print("require_replanning: " + str(require_replanning))
    print("---")
    
    # current_action_plan, require_replanning = extract_plan(best_pos, require_replanning)
    ...
    #     if i == 0:
    #         raw_grid, state, reached_termination, reached_end_door, reward, done, info = env.step(current_action_plan[i], original_save_num)
    #     else:
    #         raw_grid, state, reached_termination, reached_end_door, reward, done, info = env.step(current_action_plan[i], 0)

    # latest_save_num = latest_save_num + 1
    
    # env.step(main_actions_data["stay_still"]["action"], -latest_save_num)

    best_pos, furthest_pos, current_starting_pos_x, pos_pool, visited_states, latest_save_num = start_search(env, state, latest_save_num, latest_save_num, steps_per_search)
    ...
    if state[37] > 0:
        best_pos, furthest_pos, current_starting_pos_x, pos_pool, visited_states, latest_save_num = start_search(env, original_state, original_save_num, latest_save_num, steps_per_search)
    ...
    print("4 current_action_plan: " + str(current_action_plan)) 
    return current_action_plan, current_starting_pos_x, action, best_pos, furthest_pos, pos_pool, visited_states, ticks_before_replanning, latest_save_num
...
if __name__ == "__main__":
    ...
    for run in range(args.run):
        ...
        try:
            ...
            main_best_pos = None
            main_furthest_pos = None
            main_current_starting_pos_x = 0
            main_pos_pool = None
            main_visited_states = []
            main_require_replanning = False
            main_current_action_plan = []
            main_ticks_before_replanning = 0
            ...
            while True:   
                ...
                # action, best_pos, 
                # furthest_pos, pos_pool, 
                # visited_states, ticks_before_replanning, 
                # latest_save_num

                main_current_action_plan = []
                main_current_action_plan, main_current_starting_pos_x, main_action, main_best_pos, main_furthest_pos, main_pos_pool, main_visited_states, main_ticks_before_replanning, main_new_save_load_num = optimise(main_env, main_best_pos, main_furthest_pos, main_pos_pool, main_current_starting_pos_x, main_state, main_new_save_load_num, main_visited_states, main_require_replanning, main_current_action_plan, main_ticks_before_replanning)
                # main_env.step(main_actions_data["stay_still"]["action"], main_save_load_num)
                # print("Load to intial pos, main_save_load_num: " + str(main_save_load_num))
                # keyboard.wait("space") 
                # print("main_action: " + str(main_action))
                print("main_action: " + str(main_best_pos.action))
                ...
                if main_current_action_plan == None:
                    ...
                    for i, part_action in enumerate(main_current_action_plan):
                        ...
                        test_damage += main_state[19]
                        test_death += main_state[37]
                        print("PART ACTION: " + str(part_action))
                        print("PART DAMAGE: " + str(test_damage))
                        print("PART DEATH: " + str(test_death))
                ...
                test_damage += test_state[19]
                test_death += test_state[37]
                print("Save")
                print("Save DAMAGE: " + str(test_damage))
                print("Save DEATH: " + str(test_death))
                keyboard.wait("space")      
                ...
                test_death += test_state[37]
                print("Load")
                print("Load DAMAGE: " + str(test_damage))
                print("Load DEATH: " + str(test_death))
                keyboard.wait("space")                      
