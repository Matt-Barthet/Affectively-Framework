...
def round_to_nearest_5(lst):
    rounded_list = [round(x / 3) * 3 for x in lst]
    return rounded_list


class Heist_Environment(BaseEnvironment, ABC):
    ...
    def __init__(self, id_number, graphics, weight, logging=True):
        ...
        try:
            super().__init__(id_number=id_number, graphics=graphics,
                             ...
                             path="./Builds/MS_Heist/Top-Down Shooter.exe",
    ...
    def calculate_reward(self, state, position, action):
        ...
        position = round_to_nearest_5(position)
